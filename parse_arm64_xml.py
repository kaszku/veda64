...
class ARM64XMLParser:
    ...
    def _generate_instruction_header(self, output_file: Path):
        ...
        code.append("    Condition,          // Condition code (EQ, NE, etc.)")
    ...
    def _generate_veda64_implementation(self, output_file: Path):
        ...
        code.append("        ")
        code.append("        case OperandType::Condition:")
        code.append("            // Format condition codes")
        code.append("            {")
        code.append("                const char* conds[] = {\"eq\", \"ne\", \"cs\", \"cc\", \"mi\", \"pl\", \"vs\", \"vc\",")
        code.append("                                       \"hi\", \"ls\", \"ge\", \"lt\", \"gt\", \"le\", \"al\", \"nv\"};")
        code.append("                if (value < 16) return conds[value];")
        code.append("                return \"cond\" + std::to_string(value);")
        code.append("            }")
        code.append("        ")
    ...
    def _generate_veda64_header(self, output_file: Path):
        ...
        code.append("// The decode functions accept values in memory order (as seen in hex dumps)")
        code.append("// e.g., bytes {0x7f, 0x23, 0x03, 0xd5} in memory -> pass as 0x7f2303d5")
        code.append("")
        code.append("// Pack 4 bytes into uint32_t preserving visual order (for decode functions)")
        code.append("// {0x7f, 0x23, 0x03, 0xd5} -> 0x7f2303d5")
        code.append("inline uint32_t from_bytes(const uint8_t* bytes) {")
        code.append("    return (static_cast<uint32_t>(bytes[0]) << 24) |")
        code.append("           (static_cast<uint32_t>(bytes[1]) << 16) |")
        code.append("           (static_cast<uint32_t>(bytes[2]) << 8) |")
        code.append("           static_cast<uint32_t>(bytes[3]);")
        code.append("}")
        code.append("")
        code.append("// Unpack uint32_t to 4 bytes preserving visual order")
        code.append("// 0x7f2303d5 -> {0x7f, 0x23, 0x03, 0xd5}")
        code.append("inline void to_bytes(uint32_t insn, uint8_t* bytes) {")
        code.append("    bytes[0] = static_cast<uint8_t>(insn >> 24);")
        code.append("    bytes[1] = static_cast<uint8_t>(insn >> 16);")
        code.append("    bytes[2] = static_cast<uint8_t>(insn >> 8);")
        code.append("    bytes[3] = static_cast<uint8_t>(insn);")
        code.append("}")
        code.append("")
    ...
    def _generate_decode_declaration(self, class_name: str) -> List[str]:
        ...
        code.append(f"std::optional<Instruction> {func_name}(uint32_t insn);")
        code.append("")
        code.append(f"// Decode from 4 bytes in native ARM64 memory order")
    ...
    def generate_hook_files(self, include_dir: Path, lib_dir: Path):
        """Generate hook.hpp and hook.cpp for Windows ARM64 API hooking."""
        self._generate_hook_header(include_dir)
        self._generate_hook_implementation(lib_dir)

    def _generate_hook_header(self, include_dir: Path):
        """Generate hook.hpp header file."""
        code = []
        code.append("// veda64 API Hooking for Windows ARM64")
        code.append("// Inline hooking with automatic instruction relocation")
        code.append("// Auto-generated by parse_arm64_xml.py")
        code.append("#pragma once")
        code.append("")
        code.append("// Hook support is only available on Windows")
        ...
        code.append("#define VEDA64_HOOK_API __declspec(dllexport)")
        code.append("#else")
        code.append("#define VEDA64_HOOK_API")
        code.append("#endif")
        code.append("")
        code.append("namespace veda64 {")
        ...
        code.append("VEDA64_HOOK_API const char* status_to_string(HookStatus status);")
        code.append("#endif")
        code.append("")
        ...
        code.append("VEDA64_HOOK_API void set_config(const HookConfig& config);")
        code.append("VEDA64_HOOK_API HookConfig get_config();")
        code.append("")
        code.append("// Initialize the hooking subsystem")
        code.append("// Must be called before any other hook functions")
        code.append("VEDA64_HOOK_API HookStatus initialize();")
        code.append("")
        code.append("// Shutdown the hooking subsystem")
        code.append("// Automatically removes all installed hooks")
        code.append("VEDA64_HOOK_API void shutdown();")
        code.append("")
        code.append("// Check if hooking subsystem is initialized")
        code.append("VEDA64_HOOK_API bool is_initialized();")
        code.append("")
        code.append("// Install an inline hook")
        ...
        code.append("// Returns: handle to the hook, or nullptr on failure")
        code.append("VEDA64_HOOK_API HookHandle install(void* target, void* detour, void** original);")
        code.append("")
        code.append("// Install hook with status output")
        code.append("VEDA64_HOOK_API HookHandle install_ex(void* target, void* detour, void** original, HookStatus* status);")
        code.append("")
        code.append("// Remove a previously installed hook")
        code.append("// Restores original bytes and frees trampoline")
        code.append("VEDA64_HOOK_API HookStatus remove(HookHandle handle);")
        code.append("")
        code.append("// Remove all hooks")
        code.append("VEDA64_HOOK_API void remove_all();")
        code.append("")
        code.append("// Enable a disabled hook")
        code.append("VEDA64_HOOK_API HookStatus enable(HookHandle handle);")
        code.append("")
        code.append("// Disable a hook (restores original bytes but keeps trampoline)")
        code.append("VEDA64_HOOK_API HookStatus disable(HookHandle handle);")
        code.append("")
        code.append("// Check if hook is enabled")
        code.append("VEDA64_HOOK_API bool is_enabled(HookHandle handle);")
        code.append("")
        code.append("// Get hook information")
        code.append("VEDA64_HOOK_API void* get_target(HookHandle handle);")
        code.append("VEDA64_HOOK_API void* get_detour(HookHandle handle);")
        code.append("VEDA64_HOOK_API void* get_trampoline(HookHandle handle);")
        code.append("")
        code.append("// Get number of bytes overwritten at target")
        code.append("VEDA64_HOOK_API size_t get_hook_size(HookHandle handle);")
        code.append("")
        code.append("// Get number of instructions relocated to trampoline")
        code.append("VEDA64_HOOK_API size_t get_relocated_count(HookHandle handle);")
        code.append("")
        code.append("#ifndef VEDA64_NO_STRINGS")
        code.append("// Debug: Dump hook information")
        code.append("VEDA64_HOOK_API void dump_hook(HookHandle handle);")
        code.append("#endif")
        code.append("")
        code.append("// ============================================================================")
        ...
        code.append("")
        code.append("// ARM64 jump sequences")
        code.append("// Minimum hook size is 16 bytes (LDR X16, [PC+8]; BR X16; .quad target)")
        code.append("")
        ...
        code.append("VEDA64_HOOK_API size_t generate_jump(uint8_t* buffer, void* target);")
        code.append("")
        code.append("// Generate a call sequence (saves return address)")
        code.append("// buffer: output buffer (must be at least 20 bytes)")
        code.append("// target: destination address")
        code.append("// Returns: number of bytes written")
        code.append("VEDA64_HOOK_API size_t generate_call(uint8_t* buffer, void* target);")
        code.append("")
        code.append("// Check if an instruction is PC-relative")
        code.append("VEDA64_HOOK_API bool is_pc_relative(uint32_t insn);")
        code.append("")
        code.append("// Check if an instruction can be safely relocated")
        code.append("VEDA64_HOOK_API bool can_relocate(uint32_t insn);")
        code.append("")
        code.append("// Relocate a single instruction to a new address")
        code.append("// insn: original instruction encoding")
        code.append("// old_pc: original PC value")
        code.append("// new_pc: new PC value")
        code.append("// out_insn: receives relocated instruction(s)")
        code.append("// out_count: receives number of output instructions")
        code.append("// Returns: true if relocation successful")
        code.append("VEDA64_HOOK_API bool relocate_instruction(")
        code.append("    uint32_t insn,")
        ...
        code.append("VEDA64_HOOK_API void* alloc_executable(size_t size);")
        code.append("")
        code.append("// Free executable memory")
        code.append("VEDA64_HOOK_API void free_executable(void* ptr, size_t size);")
        code.append("")
        code.append("// Make memory region writable temporarily")
        code.append("// Returns previous protection value")
        code.append("VEDA64_HOOK_API uint32_t make_writable(void* addr, size_t size);")
        code.append("")
        code.append("// Restore memory protection")
        code.append("VEDA64_HOOK_API void restore_protection(void* addr, size_t size, uint32_t old_protect);")
        code.append("")
        code.append("// Flush instruction cache")
        code.append("VEDA64_HOOK_API void flush_icache(void* addr, size_t size);")
        code.append("")
        code.append("// Suspend all threads except current (for safe hook installation)")
        code.append("VEDA64_HOOK_API void suspend_threads();")
        code.append("")
        code.append("// Resume all suspended threads")
        code.append("VEDA64_HOOK_API void resume_threads();")
        code.append("")
        code.append("} // namespace Detail")
        code.append("")
        code.append("// ============================================================================")
        code.append("// Convenience macros")
        code.append("// ============================================================================")
        code.append("")
        code.append("// Helper macro to declare original function pointer")
        code.append("#define VEDA64_DECLARE_ORIGINAL(name, ret, ...) \\")
        code.append("    using name##_t = ret(__VA_ARGS__); \\")
        code.append("    static name##_t* name##_original = nullptr;")
        code.append("")
        code.append("// Helper macro to install hook")
        code.append("#define VEDA64_INSTALL_HOOK(name, target) \\")
        code.append("    veda64::Hook::install((void*)(target), (void*)&name##_hook, (void**)&name##_original)")
        code.append("")
        code.append("// Helper macro to call original function")
        code.append("#define VEDA64_CALL_ORIGINAL(name, ...) \\")
        code.append("    (name##_original ? name##_original(__VA_ARGS__) : (decltype(name##_original(__VA_ARGS__)))0)")
        code.append("")
        code.append("} // namespace Hook")
        code.append("} // namespace veda64")
        code.append("")
        ...
        output_file = include_dir / "hook.hpp"
        self._write_file(output_file, code)
        print(f"Generated {output_file.name}")
    ...
    def _generate_hook_implementation(self, lib_dir: Path):
        ...
        code.append("")
        code.append("// Only compile on Windows")
        code.append("#if defined(_WIN32) || defined(VEDA64_HOOK_SUPPORT)")
        code.append("")
        ...
        code.append("}")
        code.append("")
        code.append("HookHandle install_ex(void* target, void* detour, void** original, HookStatus* status) {")
        code.append("    if (!g_state.initialized) {")
        code.append("        *status = HookStatus::NotInitialized;")
        code.append("        return nullptr;")
        code.append("    }")
        code.append("")
        code.append("    if (!target) {")
        code.append("        *status = HookStatus::InvalidTarget;")
        code.append("        return nullptr;")
        code.append("    }")
        code.append("")
        code.append("    if (!detour) {")
        code.append("        *status = HookStatus::InvalidDetour;")
        code.append("        return nullptr;")
        code.append("    }")
        code.append("")
        code.append("    std::lock_guard<std::mutex> lock(g_state.mutex);")
        ...
        code.append("            *status = HookStatus::HookAlreadyInstalled;")
        code.append("            return nullptr;")
        code.append("        }")
        code.append("    }")
        code.append("")
        ...
        code.append("    ctx->trampoline = create_trampoline(target, ctx->hook_size, status);")
        code.append("    if (!ctx->trampoline.code) {")
        code.append("        delete ctx;")
        code.append("        return nullptr;")
        code.append("    }")
        code.append("")
        code.append("    // Generate hook jump sequence")
        ...
        code.append("    }")
        code.append("")
        code.append("    uint32_t old_protect = Detail::make_writable(target, ctx->hook_size);")
        code.append("    std::memcpy(target, ctx->hook_bytes, ctx->hook_size);")
        code.append("    Detail::restore_protection(target, ctx->hook_size, old_protect);")
        code.append("    Detail::flush_icache(target, ctx->hook_size);")
        code.append("")
        code.append("    if (g_state.config.thread_safe) {")
        ...
        code.append("}")
        code.append("")
        code.append("HookHandle install(void* target, void* detour, void** original) {")
        code.append("    HookStatus status;")
        code.append("    return install_ex(target, detour, original, &status);")
        code.append("}")
        code.append("")
        code.append("// ============================================================================")
    ...
    def _generate_hook_examples_cpp(self, test_dir: Path):
        ...
        code.append("#include \"veda64.hpp\"")
        code.append("")
        code.append("#if defined(_WIN32) && (defined(_M_ARM64) || defined(__aarch64__))")
        code.append("#include \"hook.hpp\"")
        code.append("#include <iostream>")
        code.append("#include <string>")
        code.append("#include <vector>")
        ...
        code.append("// ============================================================================")
        code.append("namespace LoggingHook {")
        code.append("")
        ...
        code.append("    Hook::HookStatus status;")
        code.append("    hook_handle = Hook::install_ex(target, reinterpret_cast<void*>(&detour_func),")
        code.append("                                   reinterpret_cast<void**>(&original_func), &status);")
        code.append("    return hook_handle != nullptr;")
        code.append("}")
        code.append("")
        ...
        code.append("// ============================================================================")
        code.append("namespace ValidationHook {")
        code.append("")
        ...
        code.append("    Hook::HookStatus status;")
        code.append("    hook_handle = Hook::install_ex(target, reinterpret_cast<void*>(&detour_alloc),")
        code.append("                                   reinterpret_cast<void**>(&original_alloc), &status);")
        code.append("    return hook_handle != nullptr;")
        code.append("}")
        code.append("")
        ...
        code.append("// ============================================================================")
        code.append("namespace ReturnModifyHook {")
        code.append("")
        ...
        code.append("// ============================================================================")
        code.append("namespace StringHook {")
        code.append("")
        ...
        code.append("// ============================================================================")
        code.append("namespace ProfilingHook {")
        code.append("")
        ...
        code.append("// ============================================================================")
        code.append("namespace ConditionalHook {")
        code.append("")
        ...
        code.append("    if (!target) return false;")
        code.append("    Hook::HookStatus status;")
        code.append("    hook_handle = Hook::install_ex(target, reinterpret_cast<void*>(&detour_sleep),")
        code.append("                                   reinterpret_cast<void**>(&original_sleep), &status);")
        code.append("    return hook_handle != nullptr;")
        code.append("}")
        code.append("")
        code.append("void uninstall() { if (hook_handle) { Hook::remove(hook_handle); hook_handle = nullptr; } }")
        ...
        # Example 7: Macro Example implementation
        code.append("// ============================================================================")
        code.append("// MacroExample Implementation")
        code.append("// ============================================================================")
        code.append("namespace MacroExample {")
        code.append("")
        code.append("VEDA64_DECLARE_ORIGINAL(MessageBoxA, int, HWND, LPCSTR, LPCSTR, UINT)")
        code.append("static Hook::HookHandle hook_handle = nullptr;")
        code.append("")
        code.append("static int MessageBoxA_hook(HWND hWnd, LPCSTR lpText, LPCSTR lpCaption, UINT uType) {")
        code.append("    std::string new_caption = \"[Hooked] \";")
        code.append("    new_caption += lpCaption ? lpCaption : \"Message\";")
        code.append("    return VEDA64_CALL_ORIGINAL(MessageBoxA, hWnd, lpText, new_caption.c_str(), uType);")
        code.append("}")
        code.append("")
        code.append("bool install() {")
        code.append("    HMODULE user32 = GetModuleHandleA(\"user32.dll\");")
        code.append("    if (!user32) user32 = LoadLibraryA(\"user32.dll\");")
        code.append("    if (!user32) return false;")
        code.append("    void* target = GetProcAddress(user32, \"MessageBoxA\");")
        code.append("    if (!target) return false;")
        code.append("    hook_handle = VEDA64_INSTALL_HOOK(MessageBoxA, target);")
        code.append("    return hook_handle != nullptr;")
        code.append("}")
        code.append("")
        code.append("void uninstall() { if (hook_handle) { Hook::remove(hook_handle); hook_handle = nullptr; } }")
        ...
        # Example 8: Hook Manager class
        code.append("// ============================================================================")
        code.append("// Example 8: HookManager")
        ...
        code.append("        Hook::HookStatus status;")
        code.append("        auto h = Hook::install_ex(target, detour, original, &status);")
        code.append("        if (!h) return false;")
        ...
        code.append("} // namespace veda64")
        code.append("")
        code.append("#endif // Windows ARM64")
        code.append("")
