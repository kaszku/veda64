...
void test_add_32_addsub_imm() {
    ...
    if (!result.has_value()) { std::cerr << "DECODE FAIL: add_32_addsub_imm" << std::endl; throw std::runtime_error("decode failed"); }
...
void test_add_64_addsub_imm() {
    ...
    auto result = decode(insn);
    if (!result.has_value()) { std::cerr << "DECODE FAIL: add_64_addsub_imm" << std::endl; throw std::runtime_error("decode failed"); }
    assert(result->mnemonic == Mnemonic::ADD);
...
void test_mov_add_64_addsub_imm() {
    uint32_t insn = 0x91000000u;
    auto result = decode(insn);
    if (!result.has_value()) { std::cerr << "DECODE FAIL: mov_add_64_addsub_imm" << std::endl; throw std::runtime_error("decode failed"); }
    assert(result->mnemonic == Mnemonic::ADD);
...
void test_movk_32_movewide() {
    ...
    auto result = decode(insn);
    if (!result.has_value()) { std::cerr << "DECODE FAIL: movk_32_movewide" << std::endl; throw std::runtime_error("decode failed"); }
...
void test_movk_64_movewide() {
    ...
    auto result = decode(insn);
    if (!result.has_value()) { std::cerr << "DECODE FAIL: movk_64_movewide" << std::endl; throw std::runtime_error("decode failed"); }
    assert(result->mnemonic == Mnemonic::MOVK);
    assert(result->operands.size() >= 2);
    assert(result->operands[0].type == OperandType::Register);
    assert(result->operands[1].type == OperandType::RegisterList);
