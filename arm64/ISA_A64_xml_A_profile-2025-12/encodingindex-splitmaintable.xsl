...
<td><div class="topbar"><a href="notice.xml">Proprietary Notice</a></div></td>
  ...
  <xsl:template name="decode_table">
    ...
    <xsl:variable name="VISIBLE_COLUMN_NAMES">
      <xsl:for-each select="regdiagram/box[@name and @usename='1']">
        <xsl:variable name="NAME" select="@name"/>
        <xsl:if test="count($CHILDREN/decode/box[@name = $NAME][normalize-space(.) != '' and normalize-space(.) != '-']) &gt; 0">
          <xsl:value-of select="concat('|', $NAME, '|')"/>
        </xsl:if>
      </xsl:for-each>
    </xsl:variable>
    <xsl:variable name="VISIBLE_COLUMN_COUNT" select="count(regdiagram/box[@name and @usename='1'][contains($VISIBLE_COLUMN_NAMES, concat('|', @name, '|'))])"/>
    ...
    <div class="instructiontable">
      <table class="instructiontable">
        <tr>
          ...
          -->
          <xsl:if test="$VISIBLE_COLUMN_COUNT &gt; 0">
            <xsl:element name="th">
              <xsl:attribute name="colspan">
                <xsl:value-of select="$VISIBLE_COLUMN_COUNT"/>
              </xsl:attribute>
          ...
          <th>
            <xsl:attribute name="rowspan">
              <xsl:choose>
                <xsl:when test="$VISIBLE_COLUMN_COUNT &gt; 0">2</xsl:when>
        ...
        <xsl:if test="$VISIBLE_COLUMN_COUNT &gt; 0">
          <tr>
            <xsl:for-each select="regdiagram/box[@name and @usename='1'][contains($VISIBLE_COLUMN_NAMES, concat('|', @name, '|'))]">
  ...
  <xsl:key name="by-node-id" match="node" use="concat(string(@iclass), '|', string(@groupname))"/>
  <xsl:template match="node">
    ...
    <xsl:apply-templates select="node[generate-id() = generate-id(key('by-node-id', concat(string(@iclass), '|', string(@groupname)))[1])]"/>
  </xsl:template>
    ...
    <div class="iclass" id="iclass-{@id}">
      <xsl:choose>
        <xsl:when test="count(./regdiagram) > 1">
          ...
          <xsl:element name="{concat('h', $depth)}">
            <xsl:attribute name="class">iclass</xsl:attribute>
            <xsl:value-of select="@title"/>
          </xsl:element>
        </xsl:when>
        <xsl:otherwise>
